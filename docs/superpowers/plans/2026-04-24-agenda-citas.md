# Agenda de Citas — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Reemplazar el placeholder de `Citas.jsx` con el módulo completo de agenda: lista filtrable, panel lateral para crear/editar citas, badge clickeable para cambiar estado.

**Architecture:** Un solo archivo `frontend/src/pages/Citas.jsx` con componentes internos (`BarraFiltros`, `TablaCitas`, `DropdownEstado`, `PanelNuevaCita`). El backend ya está completo — solo se construye el frontend. El panel lateral se anima con Tailwind `translate-x`. El cambio de estado llama a `PATCH /api/citas/:id/estado` y actualiza solo la fila afectada en el estado local.

**Tech Stack:** React 18, Tailwind CSS 3, lucide-react, axios (ya configurado en `src/api/axios.js`)
...
|---|---|---|
| `frontend/src/pages/Citas.jsx` | **Reemplazar** | Módulo completo (lista + panel + dropdown) |

Un solo archivo. No se crean componentes externos — el spec define todo interno.

---

## Task 1: Esqueleto del módulo con estado y carga de datos

**Files:**
- Modify: `frontend/src/pages/Citas.jsx`

- [ ] **Step 1: Reemplazar el placeholder con el esqueleto**
...
Abre http://localhost:3000/citas — debe mostrar "Cargando módulo... (0 citas)" sin errores en consola.
...
git commit -m "feat(citas): esqueleto con estado y carga de datos"
```

---

## Task 2: Barra de filtros y cabecera

**Files:**
- Modify: `frontend/src/pages/Citas.jsx`

- [ ] **Step 1: Añadir componente BarraFiltros y cabecera del módulo**

Reemplazar el `return` del componente `Citas` por:
...
Abre http://localhost:3000/citas — deben verse los pills de filtro (Todas/Hoy/Esta semana), el dropdown de estado y el botón "+ Nueva Cita". Al cambiar filtros, la consola de red debe mostrar llamadas a `/api/citas` con los parámetros correctos.

- [ ] **Step 3: Commit**

```bash
git add frontend/src/pages/Citas.jsx
git commit -m "feat(citas): barra de filtros y cabecera"
```

---

## Task 3: Tabla de citas y estado vacío

**Files:**
- Modify: `frontend/src/pages/Citas.jsx`

- [ ] **Step 1: Añadir funciones helper y la tabla**

Antes del `return`, añadir la función `eliminar`:
...
- [ ] **Step 2: Reemplazar `<p className="text-gray-400 text-sm">Tabla de citas aquí...</p>` por la tabla completa**
          ...
          <div className="divide-y divide-gray-50">
            {citas.map((cita) => (
              ...
              >
                ...
                {/* Estado — badge clickeable, placeholder por ahora */}
                <div>
                  <span className={`inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1 rounded-full cursor-pointer select-none ${ESTADO_CONFIG[cita.estado]?.badge}`}>
...
Con el backend corriendo, abrir http://localhost:3000/citas. Debe mostrarse el estado vacío con el emoji 📅. Si hay citas en la DB (de pruebas anteriores), aparecen en la tabla con columnas correctas.

- [ ] **Step 4: Commit**

```bash
git add frontend/src/pages/Citas.jsx
git commit -m "feat(citas): tabla de citas con estado vacío"
```

---

## Task 4: Dropdown de cambio de estado

**Files:**
- Modify: `frontend/src/pages/Citas.jsx`

- [ ] **Step 1: Añadir función `cambiarEstado` y ref para detectar clic fuera**

Añadir dentro del componente `Citas`, antes del `return`:
...
- [ ] **Step 2: Reemplazar el badge de estado estático por el dropdown interactivo**

Reemplazar el bloque `{/* Estado — badge clickeable, placeholder por ahora */}` por:
...
```jsx
                ...
                <div className="relative" ref={dropdownEstadoId === cita.id ? dropdownRef : null}>
                  <span
                    ...
                    className={`inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1 rounded-full cursor-pointer select-none ${ESTADO_CONFIG[cita.estado]?.badge} ${TRANSICIONES[cita.estado]?.length > 0 ? 'hover:opacity-80' : 'cursor-default'}`}
                  >
                    <span className={`w-1.5 h-1.5 rounded-full ${ESTADO_CONFIG[cita.estado]?.dot}`} />
                    {ESTADO_CONFIG[cita.estado]?.label}
...
- [ ] **Step 3: Verificar dropdown en el navegador**

Crear una cita de prueba vía curl:
```bash
curl -s -X POST http://localhost:3001/api/citas \
  -H "Content-Type: application/json" \
  -d '{"mascota_id":1,"veterinario":"Dr. García","fecha":"'$(date +%Y-%m-%d)'","hora":"10:00","motivo":"Control rutinario"}'
```

Abrir http://localhost:3000/citas, hacer clic en el badge "Pendiente" — debe abrir menú con Confirmada / Atendida / Cancelada. Seleccionar uno y verificar que cambia sin recargar la página. Clic fuera debe cerrar el menú.

- [ ] **Step 4: Commit**

```bash
git add frontend/src/pages/Citas.jsx
git commit -m "feat(citas): dropdown cambio de estado con rollback"
```

---

## Task 5: Panel lateral — estructura y animación

**Files:**
- Modify: `frontend/src/pages/Citas.jsx`

- [ ] **Step 1: Añadir estado del formulario y carga de mascotas**
...
**Nota:** esta función `abrirPanel` reemplaza la definida en Task 2 — asegúrate de que solo existe una declaración de `abrirPanel` en el componente.

- [ ] **Step 2: Añadir el panel lateral al final del JSX, antes del cierre del div raíz**

Al final del `return`, antes del `</div>` raíz, añadir:
...
- [ ] **Step 3: Verificar animación del panel**

Abrir http://localhost:3000/citas, hacer clic en "+ Nueva Cita" — el panel debe deslizar desde la derecha. Hacer clic en el overlay oscuro debe cerrarlo. El botón ✕ también debe cerrarlo.

- [ ] **Step 4: Commit**

```bash
git add frontend/src/pages/Citas.jsx
git commit -m "feat(citas): panel lateral con animación slide-in"
```

---

## Task 6: Formulario del panel lateral

**Files:**
- Modify: `frontend/src/pages/Citas.jsx`

- [ ] **Step 1: Añadir función `guardar`**
...
- [ ] **Step 2: Reemplazar `<p className="p-6 text-gray-400 text-sm">Formulario aquí (Task 6)...</p>` por el formulario completo**
                  ...
                  <div className="absolute top-full left-0 right-0 bg-white border border-gray-200 rounded-xl shadow-lg z-10 max-h-48 overflow-y-auto mt-1">
                    {mascotasFiltradas.map((m) => (
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          setForm((f) => ({ ...f, mascota_id: m.id, mascota_label: `${ESPECIES[m.especie] || '🐾'} ${m.nombre}` }));
...
1. Abrir http://localhost:3000/citas
2. Clic en "+ Nueva Cita" → panel se abre
3. Buscar "Firulais" en el campo mascota → debe aparecer en la lista desplegable
4. Seleccionar mascota, llenar fecha/hora/veterinario/motivo
5. Clic "Agendar" → panel se cierra, cita aparece en la tabla
6. Clic en lápiz de una cita → panel se abre con datos prellenados
7. Modificar motivo → "Guardar cambios" → tabla actualizada

- [ ] **Step 4: Commit**

```bash
git add frontend/src/pages/Citas.jsx
git commit -m "feat(citas): formulario completo con búsqueda de mascotas y autocompletado vet"
```
...
**Files:**
- Modify: `frontend/src/pages/Citas.jsx` (solo si hay issues)

- [ ] **Step 1: Prueba de flujo completo**

Verificar manualmente estos escenarios:

| Escenario | Resultado esperado |
|---|---|
| Sin backend → cargar /citas | Banner amarillo de error, lista vacía |
| Filtro "Hoy" → sin citas hoy | Estado vacío con mensaje "Sin citas para hoy" |
| Crear cita sin mascota | Error inline en panel, panel no se cierra |
| Badge "Pendiente" → clic | Dropdown con Confirmada/Atendida/Cancelada |
| Badge "Atendida" → clic | Sin dropdown (estado final) |
| Cambiar estado → fallo de red | Badge revierte, alert de error |
| Eliminar cita | Confirm dialog, cita desaparece de la lista |

- [ ] **Step 2: Verificar que el Dashboard sigue funcionando**

Abrir http://localhost:3000 — las citas creadas deben aparecer en "Citas de Hoy" si la fecha es hoy.

- [ ] **Step 3: Commit final**

```bash
git add frontend/src/pages/Citas.jsx
git commit -m "feat(citas): módulo agenda de citas completo"
```
