...
router.post('/', (req, res) => {
  try {
    ...
    let ids = [];
    try {
      ids = JSON.parse(medicamentos_ids || '[]');
    } catch {
      return res.status(400).json({ error: 'medicamentos_ids debe ser un JSON válido' });
    }
    ...
    for (const id of ids) {
      ...
      if (producto && producto.cantidad <= 0) warnings.push(`${producto.nombre} quedó sin stock`);
    }
